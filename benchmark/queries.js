...
// Configuration
const BASE_URL = "https://kv.mpw.sh";
const MATCH_ID = "bench";
const HEADERS = {
  "X-SECRET-KEY": "your-key",
...
const queryTypes = [
  ...
  {
    name: "Find players with good health but low mana",
    query: "$[?@.health>20&&@.mana<100]",
  },
  { name: "Find winners", query: "$[?@.winner==true]" },
  {
    name: "Find players in specific positions",
    query: "$[?@.position.x>500&&@.position.y<300]",
  },
  {
    name: "Find players with recent actions",
    query: "$[?@.last_action>1000000000000]",
  },
...
export function setup() {
  ...
  const createResp = http.put(`${BASE_URL}/api/${MATCH_ID}`, null, {
    headers: HEADERS,
  });

  check(createResp, {
    "Collection created successfully": (r) =>
      r.status === 201 || r.status === 200,
  });

  if (createResp.status !== 201 && createResp.status !== 200) {
    console.error(
      `Failed to create collection: ${createResp.status} - ${createResp.body}`,
    );
  }
...
export default function () {
  ...
  if (__VU === 1) {
    ...
    if (writeResp.status !== 201 && writeResp.status !== 200) {
      console.error(
        `Failed to write batch data: ${writeResp.status} - ${writeResp.body}`,
      );
    }
  }
  ...
  if (queryResp.status !== 200) {
    console.error(
      `Failed query: ${queryResp.status} - Query: ${randomQueryType.query}`,
    );
  } else {
